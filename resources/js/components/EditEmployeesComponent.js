...
let dataEditEmployee = document.getElementById("dataEditEmployee");
if(dataEditEmployee){
    dataEditEmployee = JSON.parse(document.getElementById("dataEditEmployee").value);
}
...
function handleSubmit(event){
    ...
    let dataEmployee = {
        ...
        edit_date: getCurrentDate()
    };
    let urlEdit = `./${dataEditEmployee.id}/editEmployee`;
    axios.post(urlEdit, dataEmployee)
...
function generateEmailCidenet(event){
    ...
    let urlValidateEmail = `./${dataEditEmployee.id}/validateEmail`;
    axios.post(urlValidateEmail, {newEmail, domain}).then(function (response) {
...
function getDataEmployee(){
    document.getElementById("first_ape").value = dataEditEmployee.first_ape;
    document.getElementById("second_ape").value = dataEditEmployee.second_ape;
    document.getElementById("first_name").value = dataEditEmployee.first_name;
    document.getElementById("other_name").value = dataEditEmployee.other_name;
    document.getElementById("employ_country").value = dataEditEmployee.employ_country;
    document.getElementById("ide_type").value = dataEditEmployee.ide_type;
    document.getElementById("ide_num").value = dataEditEmployee.ide_num;
    document.getElementById("email").value = dataEditEmployee.email;
    $("#login_date").datepicker('setDate', dataEditEmployee.login_date);
    document.getElementById("area").value = dataEditEmployee.area;
    document.getElementById("status").value = dataEditEmployee.status;
}

function EditEmployeesComponent() {
    setTimeout(() => { getDataEmployee(); }, 500);
    return (
        <div className="container">
            <div className="row">
                <h2 id='titleEdit'>Edición de Empleado</h2>
                <form onSubmit={handleSubmit}>
                    ...
                    <div className="row mb-2">
                        ...
                        <div className="col">
                            ...
                            <select className="form-select" name='status' id='status' defaultValue={'DEFAULT'} 
                                data-dom-error="statusHelp" required>
                                <option value="DEFAULT" disabled>-Seleccione-</option>
                    ...
                    <button type="submit" className="btn btn-primary">Editar Empleado</button>
                </form>
            </div>
        </div>
    )
...
if (document.getElementById('EditEmployeesComponent')) {
    ReactDOM.render(<EditEmployeesComponent />, document.getElementById('EditEmployeesComponent'));
}